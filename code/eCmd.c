...
struct ECFG{
    char *etsd;      // -t ETSD file
    char *rrd;       // -r RRD file
    char *file;      // -f text file name (for export)
    char *xdbConfig; // -d
    char **chanDefs; // array of channel definitions used to create databases (either ETSD or Ext DB)
    uint32_t start;  // start= 
    uint32_t end;    // end=
    uint16_t interv; // int= interval    (for 'create')
    uint8_t chanCnt; // number of elements in chanDefs
    uint8_t uid;     // uid= unit ID     (for 'create')
    uint8_t xDataSize;   // xd= xData bytes (for 'create')
    uint8_t output;  // out= output format (for export)  (Raw/Text/RRD/CSV/XML?)
    uint8_t query;   // query= query type (Max/Min/Ave/Total...)
    uint8_t seRel;   // start/end relative incomplete reference
} Ecfg;


// test to see if the first 'size' digits of string contains only alpha numerics and/or '_'
// Returns zero(success) if true
...
int eConfig(char key, char *value){
    char elog[]="Config Error";
    int rtn=0;
    
    switch {key){
    case 'c':
    case 'C':
        realloc( Ecfg.chanDefs ,(Ecfg.chanCnt+1) * sizeof(char*) )
        Ecfg. = value;
        break;
    case 'd':
    case 'D':
        Ecfg.xdbConfig = value;
        break;
    case 'e':
    case 'E':
        Ecfg.end = etsdParseTime(value);
        if(strcasestr(value,"start")){  // if end is relative to start
            if( 0 > Ecfg.end) {         // error: end value must be positive (after) relative to start
                    ErrorCode |= E_STARTSTOP;
                    ELog(elog, 0);
                    rtn = -1;              
            } else {                    // else end must be positive or zero
                if(Ecfg.start) {
                    Ecfg.end += Ecfg.start; 
                    Ecfg.seRel = 0;
                } else {                    
                    if(Ecfg.seRel) {         // this means start time was ALSO waiting on end time
                        ErrorCode |= E_STARTSTOP;
                        ELog(elog, 0);
                        rtn = -1;
                    } else
        ...
        } else if(strcasestr(value,"end")){  //  Error Invalid, end can't be relative to end
            ErrorCode |= E_STARTSTOP;
            ELog(elog, 0);
            rtn = -1;
        } else if(Ecfg.seRel){
    ...
    case 's':
    case 'S':
        Ecfg.start = etsdParseTime(value);
        if(strcasestr(value,"end")){    // if start is relative to end
            if ( 0 < Ecfg.start ) {     // error: start value must be negative(before) relative to end
                    ErrorCode |= E_Ecfg.startSTOP;
                    ELog(elog, 0);
                    rtn = -1;
            } else {                    // start value must be negative...or zero 
                if(Ecfg.end){ 
                    Ecfg.start += Ecfg.end;
                    Ecfg.seRel = 0;
                } else {                // else no end time yet
                    if(Ecfg.seRel) {    // error: this indicates that end time was ALSO waiting on start time
                        ErrorCode |= E_Ecfg.startSTOP;
                        ELog(elog, 0);
                        rtn = -1;
                    } else
        ...
        } else if(strcasestr(value,"start")){  //  Error Invalid, start can't be relative to start
            ErrorCode |= E_Ecfg.startSTOP;
            ELog(elog, 0);
            rtn = -1;
        } else if(Ecfg.seRel){
...
create /var/db/garage.tsd u=1 i=1s GarageMain:9:E1:r Servers:15:E2:r Fridge_Freezer:8:E5:r AC_Voltage:4:E11:G Water_Heater:8:E7:r TV_Entertainment:8:E6:r Evap_Solar:8:E8:r Mini_Split:8:E9:r
...
./etsdCmd create /var/db/garage.tsd u=1 i=10s GarageMain:9:E1:r Servers:15:E2:r Fridge_Freezer:8:E5:r AC_Voltage:4:E11:G Water_Heater:8:E7:r TV_Entertainment:8:E6:r Evap_Solar:8:E8:r Mini_Split:8:E9:r
...
*/
    ...
    intervals = (BLOCKSIZE-8-xDataSize-registers*4) / (streams/4.0);

    if(127<intervals){
        intervals = 127 ;
    }  
    
    printf(" Saving %d registers | channels = %d | intervals = %d | interval time = %d seconds | bytes per interval = %.2f\n Wasted space = %d bytes.\n\n", registers, channels, intervals, intTime, streams/4.0, (BLOCKSIZE-8-xDataSize-registers*4-(int)((intervals*streams+3)/4)));
...
int32_t examinETSD(int argc, char *argv[]){
    ...
    printf ( " #   Name                type  Chan    Type     /Guage   Register  Ext DB?   Integer?\n\n");
    for (lp=0;lp<EtsdInfo.channels;lp++){
        switch(ETSD_Type(lp)){
            case 15:
        ...
        printf("%2d %-20s  %s   %2u    %-9s  %-7s     %c         %c         %c\n", lp, EtsdInfo.label[lp], SRC_TYPE(lp)?"SHM":"ECM", SRC_CHAN(lp), sType, CNT_bit(lp)?"Counter":"Guage", REG_bit(lp)?'Y':'N', EDO_bit(lp)?'Y':'N', SIGNED(lp)?'Y':'N');
        if (REG_bit(lp))
            reg++;
    }
...
int main(int argc, char *argv[]){
    char *ptr, *value, *Config;
    //char **argp;
    //char inp[20];
    int chanCnt2=0, cnt;
    // int argpc;
    LogLvl=5;
    if (1 == argc) {  // we do not have command line arguments
        //check for config file in /etc/etsd/
        //printf("%s",help);
        exit (1);
...
//        }        
    for (cnt=2;cnt<argc;cnt++){
        ...
        if (value){
            if( eConfig(*argv[cnt], ++value) ) {
                fprintf(stderr,"Error, invalid arguement: %s\n", argv[cnt]);
                exit(1);
            }
        } else if ( '-' == *argv[cnt] ){
            if ( 'c' == argv[cnt][1]) {  // config file
                FILE *fd;
                char *ptr;
                char configLine[260];
                if ( NULL == (fd = fopen(argv[cnt+1], "r")) ) {
                    Log("<3> Error! Can't open config file: %s\n", argv[cnt+1]);
                    exit(1);
                }
                while ( fscanf(fd,"%[^\n]\n", configLine) != EOF){
                    if ( strchr(configLine,'=') && 'C' == *configLine ) {
                        Ecfg.chanCnt++;
                    }
                }
                Ecfg.chanDefs = malloc( Ecfg.chanCnt * sizeof(char*) );
                Ecfg.chanCnt = 0;
                rewind(fd);
                while ( fscanf(fd,"%[^\n]\n", configLine) != EOF){
                    ...
                    if ( ptr ) {
                        ptr++;
                        if ('C' == *configLine ){
                            Ecfg.chanDefs[Ecfg.chanCnt] =  malloc( sizeof(ptr)+1 );
                            strcpy(chanDefs[Ecfg.chanCnt++], ptr); 
                        } else {
                            if( eConfig(*configLine, ptr) ) {
                                fprintf(stderr,"Error, invalid arguement: %s %s\n", argv[cnt], argv[cnt+1]);
                                exit(1);
                            }                        
                        }
                    }
                ...
                if( eConfig(argv[cnt][1], argv[cnt+1]) ) {
                    fprintf(stderr,"Error, invalid arguement: %s %s\n", argv[cnt], argv[cnt+1]);
                    exit(1);
                }
                cnt++;
            ...
            if (!Ecfg.chanCnt){
                Ecfg.chanDefs = malloc ( (argc-cnt) * sizeof(char*) );
            }
            Ecfg.chanDefs[Ecfg.chanCnt] = malloc ( sizeof(argv[cnt]) + 1 );
            strcpy( Ecfg.chanDefs[Ecfg.chanCnt++], argv[cnt] );
        }
    ...
    switch(argv[1][0]){   
        ...
        case 'r':
        case 'R':
            if(etsdInit(Ecfg.etsd, 1)){
                ELog(__func__, 0);
                exit(1);
            }
// Pete need to make this work with other external databases, perhaps dynamically load a module depending on the file extention provided on command line
//              recoverRRD(argpc, argp);  
            break;
    }
