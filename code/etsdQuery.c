...
//    if ( 2147483647<secs || -2147483648>secs){  // resulting value doesn't fit in an 32bit int
    if ((uint64_T)(secs+2147483648)>4294967295){  // resulting value doesn't fit in an 32bit int
        ErrorCode |= E_ARG;
        secs = 0;  //calling function needs to check ErrorCode 
        err=1;
    } 
...
//    sprintf(step,"--step=%d",EtsdInfo.intervalTime);
    if ('\0' == *fName)
       ...
       sprintf(rrdValues, "%s --start=%u --step=%u", fName, EARLIEST_TIME, EtsdInfo.intervalTime); 
...
int32_t recoverRRD(int argc, char *argv[], char *rrdFName, uint8_t overwrite){
    ...
    uint8_t raw=0, last=0, first=0, lastLoop;
...
//                        switch(
    ...
    for(lp=0;lp < first; lp++){     //populate etsd counters
        for(lp2=0;lp2<EtsdInfo.channels;lp2++){
...
int64_t etsdAMT(char *cmd, uint8_t chan, uint32_t start, uint32_t end){
    ...
    int64_t Tot; 
    uint32_t before=0,  after=0, head=0, tail=0, prevTime = 0, timeStamp, lastTime=EARLIEST_TIME, endTime, bump=0, intvCnt=0, sector;
