...
void ELog(const char *cfnm, uint8_t clear){
    ...
    if(0<LogLvl){       // only log errors when log level is greater than 0
        ...
        while(eCode){
            if(eCode&1){
                if(9<idx) { // '<5>Notice' are bits 1-10
                    if(14<idx)
                        eLvl=5; // '<3>Error' is levels 16-32
                    else        
                        eLvl=4;  // '<4>Warning' is levels 11-15
                } 
